...
function switchMode(mode) {
    ...
    if (state.currentSentenceIdx >= 0) {
        setActiveSentence(state.currentSentenceIdx);
    }
}
...
function renderSentences() {
    ...
    Object.keys(groups).sort((a, b) => a - b).forEach(pKey => {
        ...
        if (state.mode === 'reading') {
            groupDiv.classList.add('paragraph-reading-grid');
            const transDiv = document.createElement('div');
            transDiv.className = 'paragraph-translation-col';
            transDiv.id = `trans-col-${pKey}`;
